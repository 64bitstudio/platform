...
## Ticket 008 (2026-09-15, CERRADO): Traefik confía en el `X-Forwarded-Proto` real de nginx

**Hallazgo real** (verificación en vivo de galgoth-studio#093, "Conectar" cuenta de Google): nginx (host, único punto de entrada TLS público) ya manda `X-Forwarded-Proto: https` correcto al reenviar a Traefik. Pero el entrypoint `web` de Traefik (ticket 049) no tenía `forwardedHeaders.trustedIPs` configurado — sin eso, Traefik no confía en ningún header reenviado y lo sobreescribe con el esquema de SU PROPIA conexión entrante (`http`, porque nginx→Traefik puertas adentro es HTTP plano en `127.0.0.1:8000`) antes de reenviar al contenedor de cada servicio. No es específico de un servicio: **cualquiera** detrás de este Traefik que construya una URL absoluta desde el request hereda el mismo esquema incorrecto — el caso real que lo expuso fue el `redirect_uri` de OAuth2 de `auth-core-mc`, que Google rechazaba con `redirect_uri_mismatch` (ver `auth-core-mc/docs/ARQUITECTURA.md`, tickets 065-068).

**Dos vueltas, verificadas con `curl` real en cada una** (nunca solo "debería funcionar"): la primera (`trustedIPs: ["127.0.0.1/32"]`, la intención razonable dado que el puerto se publica como `127.0.0.1:8000:8000`) NO funcionó — un `curl` directo tras el deploy seguía devolviendo `http://`. Causa real de por qué no: Docker hace el DNAT del puerto publicado vía iptables antes de que el paquete llegue al contenedor, así que Traefik ve la conexión de nginx como originada en la gateway de la red `edge` (`172.20.0.1`, `docker network inspect edge`), nunca literalmente `127.0.0.1`, aunque desde el HOST sí sea loopback. Segunda vuelta (con VoBo explícito de Marco para ampliar el límite de confianza — el permiso de la herramienta bloqueó el primer intento del ajuste, correctamente): se agregó también `172.20.0.1/32`. Confirmado con `curl` real contra `auth-dev.64bitstudio.com`: `redirect_uri` ya trae `https://`.

**Gotcha operativo real**: `docker compose up -d` (el paso ya existente en `sync-vm-infra`) NO recrea el contenedor Traefik solo por un cambio en el archivo de config bind-montado — el hash de servicio de Compose no cambia si la imagen/comando/volúmenes no cambian. Cada vuelta necesitó un `--force-recreate traefik` manual vía SSH tras el merge. Queda documentado acá para quien vuelva a tocar `traefik.yml` y se pregunte por qué su cambio "no hizo nada" pese a CI verde.
